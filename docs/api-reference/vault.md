...
_normalised_, a consistent IIIF Presentation API 3 model, even if it started out
...
[installation instructions](../../docs/installation) for the full details. The
following examples use imports from `@iiif/helpers@4` in an application bundler.
Install it together with `@iiif/parser@4`.
...
The `@iiif/helpers` library includes other helpers, including some used in other
...
await vault.subscribe(
  ...
  (annotationPage) => {
    ...
    show(
      [getValue(annotationPage.label), annotationPage.items.length + ' items'],
      'Annotation Page',
    );
  },
);
