...
const browserType =
  process.env.EXAMPLE_BROWSER === 'webkit' ? webkit : chromium;
...
try {
  ...
  await page.route(
    (url) =>
      url.href === 'https://digirati-co-uk.github.io/wunder.json' ||
      url.href === flexManifest,
    (route) => {
  ...
  async function open(id) {
    ...
    const iframe = page.locator(
      `.docs-example-preview iframe[src$="/${id}.html"]`,
    );
  ...
  await page
    .locator('.docs-example-files button', { hasText: 'index.html' })
    .click();
  assert(
    (await page.locator('.docs-example-source pre').textContent()).includes(
      '<canvas-panel',
    ),
  );
  await page.getByRole('button', { name: 'Reset preview' }).click();
  await page.locator('.docs-example-preview iframe').waitFor();
  const resetFrame = await (
    await page.locator('.docs-example-preview iframe').elementHandle()
  ).contentFrame();
  await resetFrame.waitForFunction(() =>
    document.querySelector('canvas-panel')?.getCanvasId?.(),
  );
  ...
  await page.waitForFunction(
    (background) =>
      getComputedStyle(document.querySelector('.docs-example-header'))
  ...
  await frame.waitForFunction(
    (expected) =>
      document.querySelectorAll('input[type=checkbox]')[1].checked === expected,
    !checked,
  );
  ...
  const before = await frame
    ...
    .evaluate((panel) => panel.getCanvasId());
  await frame.locator('.thumb-list button').nth(1).click();
  await frame.waitForFunction(
    (id) => document.querySelector('canvas-panel').getCanvasId() !== id,
    before,
  );
  ...
  assert(
    (await link.getAttribute('href')).includes(
      'example-editor?id=intro-script',
    ),
  );
  ...
  assert.equal(
    await page.getByRole('button', { name: 'Edit here' }).count(),
    supportsInline ? 1 : 0,
  );
  ...
  const catalog = JSON.parse(
    await readFile('.docs-examples/catalog.json', 'utf8'),
  );
  for (const example of catalog.examples) {
    const response = await page.request.get(
      `${base}/examples/${example.id}.html`,
    );
