...
try {
  ...
  assert.deepEqual(
    await dimensions(),
    [480, 512],
    'Default viewport height must remain 512px',
  );
  await page.evaluate(() => {
  ...
  const home = await page.evaluate(() =>
    document.querySelector('#docs-check').getPosition(),
  );
  await page.evaluate(() => document.querySelector('#docs-check').zoomIn());
  await page.waitForFunction(
    (width) =>
      document.querySelector('#docs-check').getPosition().width < width,
    home.width,
  );
  ...
  const waitForBorder = (color) =>
    page.waitForFunction((color) => {
      const highlight = document
        .querySelector('#docs-check')
        .shadowRoot.querySelector('.docs-check-highlight');
  ...
  await page.mouse.move(
    (Math.max(bounds.x, canvasBounds.x) +
      Math.min(bounds.x + bounds.width, canvasBounds.x + canvasBounds.width)) /
      2,
  ...
  async function checkStaticImage(width, height) {
    ...
    const screenshot = await page
      .locator('#docs-check .atlas-container')
      .screenshot();
    ...
    assert.deepEqual(
      pixel,
      [246, 154, 53, 255],
      'Static image must actually be visible',
    );
  }
  await page.evaluate(() =>
    document.querySelector('#docs-check').setAttribute('preset', 'static'),
  );
  await checkStaticImage(360, 240);
  await page.evaluate(() =>
    document.querySelector('#docs-check').setAttribute('height', '300'),
  );
  ...
  await page.waitForFunction(() => {
    ...
    return (
      ...
      !!panel.shadowRoot.querySelector('img')
    );
  });
  await checkStaticImage(360, 240);
  await page.evaluate(async () => {
  ...
  await page.waitForFunction(
    () => !!document.querySelector('#docs-check').dataset.selection,
  );
  await page.evaluate(() => {
    const panel = document.querySelector('#docs-check');
  ...
  if (process.argv.includes('--demos')) {
    ...
    const before = await frame.evaluate(() =>
      document.querySelector('canvas-panel').getCanvasId(),
    );
    await frame.locator('.thumb-list img').nth(1).click();
    await frame.waitForFunction(
      (before) =>
        document.querySelector('canvas-panel').getCanvasId() !== before,
      before,
    );
    ...
    const initial = await frame.evaluate(
      () =>
        document.querySelector('sequence-panel').sequence.currentSequenceIndex,
    );
    ...
    await frame.waitForFunction(
      (initial) =>
        document.querySelector('sequence-panel').sequence
          .currentSequenceIndex === initial,
      initial,
    );
    ...
    console.log(
      'Passed: React choices, Vue thumbnails, sequence previous/next, standalone thumbnail navigation.',
    );
  }
