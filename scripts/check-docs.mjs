...
try {
  ...
  await page.evaluate(() => window.docsCheckHighlight.applyStyle({
    borderWidth: "2px",
    borderStyle: "solid",
    borderColor: "green",
    ":hover": { borderColor: "orange" },
  }));
  const waitForBorder = (color) => page.waitForFunction((color) => {
  ...
  async function checkStaticImage(width, height) {
    await page.waitForFunction(([width, height]) => {
      ...
      return container?.clientWidth === width && container.clientHeight === height &&
        overlay?.clientWidth === width && overlay.clientHeight === height &&
