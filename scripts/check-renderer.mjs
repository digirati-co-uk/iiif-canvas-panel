...
try {
  for (const deviceScaleFactor of [1, 2]) {
    ...
    await page.goto(`${process.env.DOCS_URL || "http://127.0.0.1:3000"}/docs/installation`);
    ...
    await page.waitForFunction(
      () => document.querySelector("#check").shadowRoot.querySelector("canvas")?.width === 600 * devicePixelRatio,
    );
    await page.evaluate(() => document.querySelector("#check").withAtlas((rt) => (window.rt = rt)));
    ...
    const before = await page.evaluate((p) => window.rt.viewerToWorld(p.x, p.y), local);
    await page.mouse.move(box.x + local.x, box.y + local.y);
    await page.mouse.wheel(0, -160);
    await page.waitForTimeout(1000);
    const after = await page.evaluate((p) => window.rt.viewerToWorld(p.x, p.y), local);
    assert((await page.evaluate(() => window.rt.width)) < widthBefore, "Wheel must actually zoom");
    assert(Math.hypot(after.x - before.x, after.y - before.y) < 2, "Pointer zoom moved the target");
    for (const preset of ["responsive", "static"]) {
      await page.evaluate((preset) => document.querySelector("#check").setAttribute("preset", preset), preset);
      await page.waitForFunction(() =>
        document.querySelector("#check").shadowRoot.querySelector(".atlas-static-image"),
      );
      const surface = page.locator("#check .atlas-static-container");
      assert.equal(await surface.evaluate((el) => getComputedStyle(el).touchAction), "auto");
