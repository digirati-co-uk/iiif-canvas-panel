...
test('every example exports the same source with a reproducible package selection', async () => {
  ...
  for (const example of examples) {
    ...
    assert.equal(JSON.parse(project.files['package.json']).dependencies['@digirati/canvas-panel-web-components'], preview);
    assert.equal(project.files['index.html'], example.files['index.html']);
    const local = exportProject(example, 'workspace', { 'dist/index.mjs': '// built library', 'dist/index.d.mts': '// declarations' });
    assert.equal(local.files['canvas-panel/dist/index.d.mts'], '// declarations');
    assert.equal(JSON.parse(local.files['package.json']).dependencies['@digirati/canvas-panel-web-components'], 'file:./canvas-panel');
  }
