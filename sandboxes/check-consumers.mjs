...
const snapshot = JSON.parse(await readFile(new URL('../.docs-examples/examples/package.json', import.meta.url), 'utf8'));
const catalog = JSON.parse(await readFile(new URL('../.docs-examples/catalog.json', import.meta.url), 'utf8'));
const spec = catalog.package === 'Local workspace build' ? 'workspace' : catalog.package;
const examples = await collectExamples();
const directory = await mkdtemp(path.join(tmpdir(), 'canvas-panel-examples-'));
try {
  for (const id of ['intro-script', 'react-choices-example', 'vue-3-carousel']) {
    ...
    if (JSON.parse(project.files['package.json']).scripts.typecheck) execFileSync('npm', ['run', 'typecheck'], { cwd, stdio: 'inherit' });
