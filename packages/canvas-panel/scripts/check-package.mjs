...
for (const format of ['import', 'require']) {
  ...
  assert(
    !declarations.includes('.build/types/'),
    'Declarations must use public dependency exports',
  );
}
...
const cjs = createRequire(import.meta.url)(
  fileURLToPath(new URL(pkg.exports['.'].require.default, root)),
);
...
const forbidden =
  /^(?:react-dom|preact|@preact|@floating-ui|polygon-editor)(?:\/|$)|^@atlas-viewer\/atlas$/;
const seen = new Set();
async function inspect(file) {
  ...
  for (const { fileName: specifier } of ts.preProcessFile(source, true, true)
    .importedFiles) {
    if (
      ['exports', 'require', 'module'].includes(specifier) ||
      isBuiltin(specifier)
    )
      continue;
    assert(
      !forbidden.test(specifier),
      `${file} imports forbidden runtime ${specifier}`,
    );
...
const script = await readFile(
  new URL(pkg.exports['./dist/index.iife.js'], root),
  'utf8',
);
assert(
  !/node_modules\/(?:react-dom|preact|@preact)\//.test(script),
  'Standalone script embeds an old DOM runtime',
);
console.log(
  'React/Atlas scene dependency graphs contain no React DOM, Preact or editor runtime.',
);
