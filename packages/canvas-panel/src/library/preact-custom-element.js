...
export default function register(Component, tagName, propNames, options) {
  function PreactElement() {
    ...
    inst._root = inst._shadowEnabled ? inst.attachShadow({ mode: 'open' }) : inst;
  ...
  const name = tagName || Component.tagName || Component.displayName || Component.name;
...
function connectedCallback() {
  ...
  this._vdom = h(ContextProvider, { ...this._props, context }, toVdom(this, this._vdomComponent, this._shadowEnabled));
...
function toVdom(element, nodeName, shadow) {
  ...
  for (i = cn.length; i--; ) {
    const comp = cn[i];
    const vnode = toVdom(comp, null, shadow);
    ...
    if (name) {
      props[name] = h(
        Slot,
        {
          name,
          shadow,
          addContextListener(listener, element = comp) {
            element.addEventListener('_preact', listener);
          },
          removeContextListener(listener, element = comp) {
