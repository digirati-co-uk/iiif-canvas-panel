...
function update(node: Element, previous: any, next: any) {
  for (const key of new Set([...Object.keys(previous), ...Object.keys(next)])) {
    if (key === 'children' || key === 'ref' || previous[key] === next[key])
      continue;
    ...
    if (key === 'style') {
      ...
      const event = key
        ...
        .toLowerCase()
        .replace('doubleclick', 'dblclick');
      if (previous[key])
        node.removeEventListener(event, previous[key], capture);
...
const remove = (parent: Node, child: Node) => {
  if (child.parentNode === parent) parent.removeChild(child);
};
const insert = (parent: Node, child: Node, before: Node) =>
...
const renderer = (Reconciler as any)({
  ...
  getChildHostContext: (namespace: string, type: string) =>
    type === 'svg' ? svg : type === 'foreignObject' ? html : namespace,
  getPublicInstance: (instance: Node) => instance,
  createInstance(type: string, props: any, root: Node, namespace: string) {
    const doc = root.ownerDocument || document;
    const node =
      type === 'svg' || namespace === svg
        ? doc.createElementNS(svg, type)
        : doc.createElement(type);
    update(node, {}, props);
    return node;
  },
  createTextInstance: (text: string, root: Node) =>
    (root.ownerDocument || document).createTextNode(text),
  ...
  resetTextContent: (node: Node) => {
    node.textContent = '';
  },
  commitUpdate: (node: Element, _type: string, previous: any, next: any) =>
    update(node, previous, next),
  commitTextUpdate: (node: Text, _old: string, text: string) => {
    node.data = text;
  },
  ...
  hideInstance: (node: HTMLElement) => {
    node.style.display = 'none';
  },
  unhideInstance: (node: Element, props: any) =>
    update(node, { style: { display: 'none' } }, props),
  hideTextInstance: (node: Text) => {
    node.data = '';
  },
  unhideTextInstance: (node: Text, text: string) => {
    node.data = text;
  },
...
export function render(
  ...
  const update = synchronous
    ? renderer.updateContainerSync
    : renderer.updateContainer;
  update(children, root, null, () => {
    if (children == null && roots.get(container)?.revision === revision)
