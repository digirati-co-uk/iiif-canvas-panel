    ...
    if (key === "style") {
      ...
      for (const name of new Set([...Object.keys(previous.style || {}), ...Object.keys(value || {})])) {
        ...
        const css =
          item == null
            ? ""
            : typeof item === "number" && item !== 0 && !unitless.has(name) && !name.startsWith("--")
              ? `${item}px`
              : String(item);
      ...
      if (value == null || (value === false && !name.startsWith("aria-") && !name.startsWith("data-")))
        node.removeAttribute(name);
      else
        node.setAttribute(
          name,
          value === true && !name.startsWith("aria-") && !name.startsWith("data-") ? "" : String(value),
        );
    }
  }
}
