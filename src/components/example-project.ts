...
export async function loadProject(base: string, id: string): Promise<Project> {
  ...
  const project: Project = await json(`projects/${encodeURIComponent(id)}.json`);
  const pkg = JSON.parse(project.files['package.json']);
  if (pkg.dependencies['@digirati/canvas-panel-web-components'] === 'file:./canvas-panel') {
    const snapshot: Record<string, string> = await json('package.json');
    for (const [name, code] of Object.entries(snapshot)) project.files[`canvas-panel/${name}`] = code;
  }
  return project;
}
