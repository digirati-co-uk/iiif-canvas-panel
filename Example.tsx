...
function ExampleCard({
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      if (active)
        setNotice(
          'The editor is taking longer than expected. You can open the project in a separate tab.',
        );
    }, 30000);
    Promise.all([import('@stackblitz/sdk'), loadProject(base, example.id)])
      .then(async ([{ default: sdk }, project]) => {
        ...
        host
          ...
          ?.setAttribute('title', `Edit ${example.title} in StackBlitz`);
        if (active) setNotice('');
      })
      .catch((error) => {
        if (active)
          setNotice(
            `Could not start the editor: ${error.message}. Try opening it in a separate tab.`,
          );
      })
  ...
  return (
    <section
      className={`docs-example docs-example--${layout}`}
      ...
      <header className="docs-example-header">
        ...
        <div className="docs-example-actions">
          ...
          {canEmbed && (
            ...
            >
              {editing ? 'Back to example' : 'Edit here'}
            </button>
          )}
          <a href={editorUrl} target="_blank" rel="noopener noreferrer">
            {editing ? 'Open original in StackBlitz ↗' : 'Open in StackBlitz ↗'}
          </a>
        </div>
      </header>
      <div className="docs-example-status" role="status">
        {notice}
      </div>
      {editing ? (
        <div className="docs-example-editor" ref={editor} />
      ) : (
        <div className="docs-example-content">
          <div
            className="docs-example-preview"
            style={{ minHeight: example.height }}
          >
            {error && (
              <div role="alert" className="docs-example-error">
                Preview error: {error}{' '}
                <button type="button" onClick={reset}>
                  Retry
                </button>
            ...
            {!running ? (
              ...
              <>
                {!loaded && (
                  <p className="docs-example-loading">Loading preview…</p>
                )}
                <iframe
                  key={revision}
                  ref={preview}
                  ...
                  onError={() => setError('Could not load the preview.')}
                />
              </>
            )}
          </div>
          <div className="docs-example-source">
            <div
              className="docs-example-files"
              role="group"
              aria-label="Example source files"
            >
              {example.visibleFiles.map((name) => (
                <button
                  type="button"
                  key={name}
              ...
              <label>
                ...
                >
                  {Object.keys(example.files).map((name) => (
